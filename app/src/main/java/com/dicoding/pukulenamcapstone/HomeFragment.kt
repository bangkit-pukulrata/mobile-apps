...
class HomeFragment : Fragment() {
    ...
    override fun onCreateView(
        ...
        // Membaca tokenizer dari file JSON
        tokenizer = readTokenizerFromAssets("tokenizer.json")

        // Memuat model TFLite dengan Flex delegate
        ...
        val newSequences = tokenizer.textsToSequences(newTexts)
        ...
        for ((index, text) in newTexts.withIndex()) {
            Log.d("HoaxDetection", "Text: $text")
            Log.d("HoaxDetection", "Prediction: ${predictions[index]}")
            Log.d("HoaxDetection", "Predicted Label: ${if (predictedLabels[index] == 1) "Hoax" else "Not Hoax"}")
        }

        return view
    }

    // Fungsi untuk membaca tokenizer dari file JSON di assets
